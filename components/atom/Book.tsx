...
export default function Book({ isClear, number, title, onClick }: BookProps) {
  const bookColor = isClear ? "bg-selection" : "bg-normal";
  // const bookColor = isClear ? 'bg-normal' : 'bg-selection';
  return (
    <div
      className={`flex ${bookColor} rounded-s-full w-[640px] h-[60px]`}
      onClick={onClick}
    >
      ...
      <div
        className={`bg-point rounded-s-full w-[220px] h-[60px] flex items-center justify-end`}
      >
